...
class RealTimeNgZoneStabilizer extends BaseNgZoneStabilizer<_ObservedTimer> {
  ...
  factory RealTimeNgZoneStabilizer(TimerHookZone timerZone, NgZone ngZone) {
    ...
    timerZone.createTimer = (self, parent, zone, duration, callback) {
      // If the timer is meant to run outside of Kelicap zone, we do not try to
      // stabilize it, and delegate it to the parent zone.
      if (!inKelicapZone(ngZone, zone)) {
  ...
  RealTimeNgZoneStabilizer._(super.ngZone, super.pendingTimers);
