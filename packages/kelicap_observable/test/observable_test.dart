...
void observableTests() {
  ...
  test('cancel listening', () {
    var t = createModel(123);
    late StreamSubscription<List<ChangeRecord>> sub;
    sub = t.changes.listen(
      expectAsync1((records) {
        expectPropertyChanges(records, 1);
        sub.cancel();
        t.value = 777;
  ...
  test('notifyPropertyChange', () {
    ...
    expect(
      t.notifyPropertyChange(#value, t.value, 42),
      42,
...
void expectPropertyChanges(List records, int number) {
  expect(records.length, number, reason: 'expected $number change records');
  for (var record in records) {
    expect(
      record is PropertyChangeRecord,
      true,
      reason: 'record should be PropertyChangeRecord',
    );
    expect(
      (record as PropertyChangeRecord).name,
      #value,
