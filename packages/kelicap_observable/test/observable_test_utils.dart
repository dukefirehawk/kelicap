...
void expectChanges(
  List<ChangeRecord> actual,
  List<ChangeRecord> expected, {
  String? reason,
}) {
  expect(actual, _EqualsMatcher(expected), reason: reason);
}

void expectNotChanges(
  List<ChangeRecord> actual,
  ChangeRecords expectedNot, {
  String? reason,
}) {
...
List<Matcher> changeMatchers(List<ChangeRecord> changes) => changes
    .map(
      (r) => r is PropertyChangeRecord ? _PropertyChangeMatcher(r) : equals(r),
    )
    .toList();
