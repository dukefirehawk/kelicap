...
void main() {
  ...
  group('observe length', () {
    ...
    test('remove changes length', () {
      ...
      return Future(() {
        expect(
          changes,
          changeMatchers([_lengthChange(map, 3, 2), _lengthChange(map, 2, 1)]),
        );
      });
    });
  ...
  group('observe item', () {
    ...
    setUp(() {
      ...
      sub = map.changes.listen((records) {
        changes = records
            .where((r) => r is MapChangeRecord && r.key == 'b')
            .toList();
      });
    });
    ...
    test('change the item', () {
      ...
      return Future(() {
        expect(changes, [_changeKey('b', 2, 9001), _changeKey('b', 9001, 42)]);
      });
    });
    ...
    test('remove and add back', () {
      ...
      expect(map, {'a': 1, 'b': 2, 'c': 3});
      return Future(() {
        expect(changes, [_removeKey('b', 2), _insertKey('b', 2)]);
      });
    });
...
PropertyChangeRecord<int> _lengthChange(
  ObservableMap map,
  int oldValue,
  int newValue,
) => PropertyChangeRecord<int>(map, #length, oldValue, newValue);

MapChangeRecord<String, int?> _changeKey(String key, int? old, int? newValue) =>
    MapChangeRecord<String, int?>(key, old, newValue);

ChangeRecord _insertKey(String key, int? newValue) =>
    MapChangeRecord<String, int?>.insert(key, newValue);

ChangeRecord _removeKey(String key, int? oldValue) =>
    MapChangeRecord<String, int?>.remove(key, oldValue);

PropertyChangeRecord<Null> _propChange(ObservableMap map, prop) =>
