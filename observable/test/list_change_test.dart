...
void listChangeTests() {
  ...
  ListChangeRecord<E> delta<E>(
    int i,
    List<E> r,
    int a, {
    ObservableList<E>? typedModel,
  }) => ListChangeRecord(
    typedModel ?? model!.map((v) => v as E).toList(),
    i,
    removed: r,
    addedCount: a,
  );
  ...
  test('sequential adds', () {
    ...
    expect(summary, null);
    return Future(() {
      expect(summary, [delta(1, [], 2, typedModel: model)]);
  ...
  test('List Splice Truncate And Expand With Length', () {
    ...
    return Future(() {
      expect(summary, [
        delta(2, ['c', 'd', 'e'], 0, typedModel: model),
      ]);
      ...
      expect(summary, [delta(2, [], 3, typedModel: model)]);
  ...
  group('edit distance', () {
    void assertEditDistance(
      orig,
      Future<List<ListChangeRecord>> changes,
      expectedDist,
