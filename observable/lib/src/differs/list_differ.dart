...
class ListDiffer<E> implements Differ<List<E>> {
  ...
  List<ListChangeRecord<E>> diff(List<E> e1, List<E> e2) {
    return _calcSplices<E>(e2, _equality, 0, e2.length, e1, 0, e1.length);
  }
}

enum _Edit { leave, update, add, delete }
...
List<ListChangeRecord<E>> _calcSplices<E>(
  ...
  if (currentStart == 0 && oldStart == 0) {
    prefixCount = _sharedPrefix(equality, current, old, minLength);
  }
  ...
  if (currentStart == currentEnd) {
    ...
    return [ListChangeRecord<E>.remove(current, currentStart, spliceRemoved)];
  }
  if (oldStart == oldEnd) {
    return [
      ListChangeRecord<E>.add(current, currentStart, currentEnd - currentStart),
    ];
  }

  final ops = _spliceOperationsFromEditDistances(
    _calcEditDistance(current, currentStart, currentEnd, old, oldStart, oldEnd),
  );
