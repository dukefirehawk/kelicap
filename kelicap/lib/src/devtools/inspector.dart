...
class Inspector {
  ...
  /// Additional locations in the DOM to search for Kelicap artifacts.
  ...
  void inspect(ApplicationRef applicationRef) {
    if (_applicationRef != null) {
      console.error(
        '''
Kelicap DevTools does not yet support apps with multiple runApp()
invocations. Please contact Kelicap-eng@ if you encounter this error.
'''
            .toJS,
      );
      return;
  ...
  ///
  /// The service extension is registered as "ext.Kelicap.[name]".
  ...
  /// Registers a service extension [handler] as "ext.Kelicap.[name]".
  void _registerServiceExtension(
    String name,
    FutureOr<Object?> Function(Map<String, String> args) handler,
  ) {
    final method = 'ext.Kelicap.$name';
    registerExtension(method, (_, args) {
      ...
      if (applicationRef != null) {
        ...
        // pass because it's called from outside the Kelicap zone.
  ...
  void _collectNodes(
    ...
    if (data != null) {
      ...
      // If this DOM node has no associated Kelicap data, we simply append any
...
/// Kelicap artifacts associated with a DOM node.
